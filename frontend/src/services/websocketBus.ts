  ...
  public async connect(): Promise<WebSocket> {
    ...
    this.ws.onmessage = (event: MessageEvent) => {
      try {
        ...
        // 🔕 Ignore noisy heartbeat & empty packets
        if (!rawMessage || Object.keys(rawMessage).length === 0) return;
        if (rawMessage.type === "ping") {
          // Optionally log at debug level every ~minute for visibility
          const now = Date.now();
          if (now % 60000 < 1000) {
            console.debug("🔁 WebSocket alive (ping).");
          }
          // Auto-reply to keep connection alive
